...
function repeatString(str, times) {
  if (typeof str === 'string' && Number.isInteger(times) && times >= 0) {
...
function countVowels(str) {
  ...
  const vowels = lowercaseStr.match(/[aeiouy]/gi);
...
function invertCase(str) {
  const invertedChars = str.split('').map((char) => {
    if (char.match(/[a-z]/)) {
...
function extractNameFromTemplate(value) {
  const cleanedValue = value.replace(/^Hello, |!$/g, '');
...
function extractEmails(str) {
  ...
  const cleanedEmails = emailList.map((email) => email.trim());
...
function encodeToRot13(str) {
  return str.replace(/[a-z]/gi, (char) => {
    ...
    const charCode = char.charCodeAt(0) - startCode;

    return String.fromCharCode(((charCode + 13) % 26) + startCode);
  });
}
